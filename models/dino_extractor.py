...
class ViTExtractor:
    ...
    def _extract_features(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        ...
        self.load_size = (H, W)
        # print('self.stride',self.stride,self.p,H,W)
        self.num_patches = (1 + (H - self.p) // self.stride[0], 1 + (W - self.p) // self.stride[1])
        return self._feats
    def _extract_DINORegfeatures(self, batch: torch.Tensor, layers: List[int] = 11, facet: str = 'key') -> List[torch.Tensor]:
        """
        extract features from the model
        :param batch: batch to extract features for. Has shape BxCxHxW.
        :param layers: layer to extract. A number between 0 to 11.
    ...
    def extract_descriptors(self, batch: torch.Tensor, layer: int = 11, facet: str = 'key',
        ...
        # print(len(self._feats))
        x = self._feats[0]
        if facet == 'token':
            x.unsqueeze_(dim=1) #Bx1xtxd
        if not include_cls:
            # print('extract',x[0,0,0,0:10],x.shape)

            x = x[:, :, 1:, :]  # remove cls token
            # print('aaaaaaaaaaaaaaaaaaaa',x.shape)
        else:
