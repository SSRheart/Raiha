...
def evaluateModel(epoch_number, model, opt, test_dataset, epoch, max_psnr, iters=None):
    ...
    eval_results = {'mask': [], 'mse': [], 'psnr': [], 'ssim':[],'mask': [], 'ref_mse': [], 'ref_psnr': [], 'ref_ssim':[]}
    ...
    for i, data in tqdm(enumerate(test_dataset)):
        ...
        for i_img in range(real.size(0)):
            ...
            ref_ssim_score = ssim(util.tensor2im(predref), util.tensor2im(gt),data_range=255,multichannel=True)
            ...
            # update calculator
            eval_results['ref_mse'].append(ref_mse_score_op)
            eval_results['ref_psnr'].append(ref_psnr_score_op)
            eval_results['ref_ssim'].append(ref_ssim_score) 
    ...
    ref_all_mse, ref_all_psnr, ref_all_ssim = calculateMean(eval_results['ref_mse']), calculateMean(eval_results['ref_psnr']),  calculateMean(eval_results['ref_ssim'])

    all_mse, all_psnr, all_ssim = calculateMean(eval_results['mse']), calculateMean(eval_results['psnr']),  calculateMean(eval_results['ssim'])
...
def resolveResults(results):
    interval_metrics = {}
    mask, mse, psnr, ssim = np.array(results['mask']), np.array(results['mse']), np.array(results['psnr']), np.array(results['ssim'])
